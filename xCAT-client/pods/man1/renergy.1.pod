...
B<renergy> command depends on the Energy Management Plugin for xCAT B<xCAT-cimclient> to 
...
=item savingstatus={on | off} 
    ...
    The B<savingstatus> only can be turned on when the B<dsavingstatus>
    is in turn off status.
...
=item dsavingstatus={on-norm | on-maxp | off} 
    ...
    The B<dsavingstatus> only can be turned on when the B<savingstatus>
    is in turn off status.
